...
public class DAOWeight {
    ...
    public boolean insert(Weight item) {
    ...
    public boolean update(Weight item) {
        ContentValues cv = new ContentValues();

        cv.put(WEIGHT_COLUMN, item.getWeight());
        cv.put(CREATEDATE_COLUMN, item.getCreateDate());

        String where = KEY_ID + "=" + item.getWeightId();
    ...
    // 把Cursor目前的資料包裝為物件
    private Weight getRecord(Cursor cursor) {
        // 準備回傳結果用的物件
        Weight result = new Weight();

        result.setWeightId((int) cursor.getLong(0));
        result.setWeight(cursor.getString(1));
