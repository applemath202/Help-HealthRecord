...
public class DAOBloodSugar {
    ...
    // 其它表格欄位名稱
    private static final String BLOODSUGAR_COLUMN = "bloodsugar";
    ...
    public static String CREATE_TABLE() {
        ...
        sb.append(BLOODSUGAR_COLUMN + " TEXT NOT NULL, ");
    ...
    // 建構子，一般的應用都不需要修改
    public DAOBloodSugar(Context context) {
    ...
    public boolean insert(BloodSugar item) {

        ContentValues cv = new ContentValues();

        cv.put(BLOODSUGAR_COLUMN, item.getBloodsugar());
    ...
    public boolean update(BloodSugar item) {
        ContentValues cv = new ContentValues();

        cv.put(BLOODSUGAR_COLUMN, item.getBloodsugar());
        cv.put(CREATEDATE_COLUMN, item.getCreateDate());

        String where = KEY_ID + "=" + item.getBloodsugarId();

        return db.update(TABLE_NAME, cv, where, null) > 0;
    }

    // 讀取所有記事資料
    public ArrayList<BloodSugar> getAll() {
    ...
    // 把Cursor目前的資料包裝為物件
    private BloodSugar getRecord(Cursor cursor) {

        // 準備回傳結果用的物件
        BloodSugar result = new BloodSugar();
        result.setBloodsugarId((int) cursor.getLong(0));
        result.setBloodsugar(cursor.getString(1));
